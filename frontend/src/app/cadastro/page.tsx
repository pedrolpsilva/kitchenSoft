...
import { createUserWithEmailAndPassword, updateProfile, signInWithPopup, signOut, getAdditionalUserInfo, updatePassword, type User } from 'firebase/auth';
...
export default function CadastroPage() {
  ...
  const handleSetGooglePassword = async (e: React.FormEvent) => {
    ...
    try {
      if (googleUser) {
        await updatePassword(googleUser, senha);
        router.push('/');
      }
    } catch (err: any) {
      console.error(err);
            ...
            <form onSubmit={handleSetGooglePassword} className="space-y-4">
              <div>
                <p className="text-sm text-zinc-300 mb-4 text-center">
                  Conta Google vinculada com sucesso! Defina uma senha para poder acessar sua conta futuramente.
                </p>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-zinc-300 mb-1" htmlFor="senha-google">
                  Senha
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-300 mb-1" htmlFor="confirmar-senha-google">
                  Confirmar Senha
                </label>
                <div className="relative">
                  <input
                    id="confirmar-senha-google"
